...
export default function TesteDetail() {
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,1fr)] lg:items-start">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {resultados.map((r) => {
                  ...
                  return (
                    <div
                      key={r.nome}
                      className={`rounded-xl border p-3 ${r.critico ? "bg-muted/30" : ""}`}
                    >
                      <p className="text-sm font-medium">{r.nome}</p>
                      <div className="mt-1.5 flex items-center gap-2">
                        <span className="text-xs whitespace-nowrap text-muted-foreground">{r.nota}/75</span>
                        <Badge
                          variant="outline"
                          className={`rounded-lg px-2 py-0.5 text-xs ${RISCO_BADGE_CLASS[risco]}`}
                        >
                          {RISCO_LABEL[risco]}
                        </Badge>
                      </div>
          ...
          <Card className="w-full py-0 shadow-sm">
            <CardHeader className="px-5 pt-5">
              <CardTitle className="flex items-center gap-2 text-sm">
                ...
                Histórico · Testes {teste.tipo} anteriores
              </CardTitle>
            </CardHeader>
            <CardContent className="px-5 pb-3">
              {historico.length === 0 ? (
                ...
                <div className="-mx-2">
                  {historico.map((t) => (
                    <div key={t.id} className="flex items-center justify-between gap-2 rounded-lg px-2 py-2 hover:bg-muted/50">
                      <span className="text-xs text-muted-foreground">{t.data}</span>
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-medium">{t.pontuacao}/100</span>
                        <Badge variant="outline" className={`rounded-lg px-1.5 py-0 text-[11px] ${RISCO_BADGE_CLASS[t.status]}`}>
                          {t.classificacao}
                        </Badge>
                      </div>
                    </div>
          ...
          <Collapsible className="w-full">
            <Card className="w-full gap-0 py-0 shadow-sm">
              <CollapsibleTrigger className="group flex w-full items-center justify-between px-5 py-4 text-left">
                <span className="text-sm font-semibold">Detalhes do funcionário</span>
                <ChevronDown className="size-4 text-muted-foreground transition-transform group-data-[state=open]:rotate-180" />
              </CollapsibleTrigger>
              <CollapsibleContent>
                <CardContent className="grid grid-cols-1 gap-4 border-t px-5 py-4">
                  <div>
